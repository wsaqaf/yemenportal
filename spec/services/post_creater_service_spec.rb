...
describe PostCreaterService do
  ...
  describe "#add_post" do
    ...
    describe "(module tests)" do
      ...
      let(:topic) { double }
      let(:tf_idf) { double(post_topic: topic) }
      ...
      it "post call save action" do
        allow(Post).to receive(:new).with(description: "description", link: "link", published_at: item.pubDate,
          source: source, title: item.title, photo_url: "some_path", topic: nil).and_return(post)
      ...
      it "post with approve state" do
        allow(Post).to receive(:new).with(description: "description", link: "link", published_at: item.pubDate,
          title: item.title, source: whitelisted_source, photo_url: "some_path", topic: nil).and_return(post)
      ...
      it "not set category" do
        allow(TfIdfService).to receive(:new).and_return(tf_idf)
        allow(Post).to receive(:new).with(description: "description", link: "link", published_at: item.pubDate,
          title: item.title, source: source, photo_url: "some_path", topic: topic).and_return(post)
        allow(source).to receive(:category).and_return(nil)
